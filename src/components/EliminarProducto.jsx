...
function EliminarProducto() {
  ...
  const eliminarProducto = async () => {
    ...
    setCargando(true);
    try {
      const respuesta = await fetch(`https://68ed704ddf2025af780033c1.mockapi.io/api/productos/${producto.id}`, {
        method: 'DELETE',
      });
     
      if (!respuesta.ok) {// Si la respuesta no esta ok, generamos el texto del error.
